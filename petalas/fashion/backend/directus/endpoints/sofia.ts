...
export default defineEndpoint((router, context) => {
  ...
  router.post('/search', async (req, res) => {
    try {
      ...
      const itemsService = new services.ItemsService('products', {
        schema,
      ...
      const products = await itemsService.readByQuery({
        filter: {
          _and: [
            { status: { _eq: 'published' } },
            { _or: [{ name: { _contains: query } }, { description: { _contains: query } }] },
          ],
        },
        limit: 20,
      });
  ...
  router.post('/checkout-assist', async (req, res) => {
    try {
      ...
      const stepMessages: any = {
        shipping:
          'Por favor, preencha suas informações de entrega. Vou garantir que seu pedido chegue rapidamente! 📦',
      ...
      return res.json({
        ...
        step: step,
        suggestions:
          step === 'shipping' ? ['Aplicar cupom', 'Calcular frete'] : ['Voltar', 'Continuar'],
      });
    } catch (error: any) {
