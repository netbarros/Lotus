...
export default defineEndpoint((router, { database }) => {
  ...
  router.post('/', async (req, res) => {
    try {
      const { tenant_id, restaurant_id, customer_name, customer_email, customer_phone,
        reservation_date, reservation_time, party_size, special_requests, occasion } = req.body;
  ...
  router.get('/:id', async (req, res) => {
    try {
      const reservation = await database('reservations')
