...
export default defineEndpoint((router, context) => {
  ...
  router.post('/reservation', async (req, res) => {
    try {
      ...
      const reservationsService = new services.ItemsService('reservations', { schema, accountability: req.accountability })
  ...
  router.post('/menu-recommendations', async (req, res) => {
    try {
      ...
      const schema = await getSchema()
      const menuItemsService = new services.ItemsService('menu_items', { schema, accountability: req.accountability })
  ...
  router.post('/order', async (req, res) => {
    try {
      ...
      return res.json({
        ...
        order,
        estimated_time: order_type === 'dine-in' ? '15-20 min' : order_type === 'takeout' ? '20-30 min' : '40-50 min',
  ...
  router.post('/table-availability', async (req, res) => {
    try {
      ...
      const schema = await getSchema()
      const tablesService = new services.ItemsService('tables', { schema, accountability: req.accountability })
      const reservationsService = new services.ItemsService('reservations', { schema, accountability: req.accountability })
      ...
      const suitableTables = await tablesService.readByQuery({
        filter: {
          ...
          _or: [
            { min_capacity: { _null: true } },
            { min_capacity: { _lte: party_size } }
          ]
        },
        sort: ['capacity']
      })
