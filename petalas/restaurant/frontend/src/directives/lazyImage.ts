...
export const vLazyImage: Directive = {
  mounted(el: HTMLImageElement, binding: DirectiveBinding<string | LazyImageOptions>) {
    const options: LazyImageOptions = typeof binding.value === 'string'
      ? { src: binding.value }
      : binding.value
        ...
        for (const entry of entries) {
          if (entry.isIntersecting) {
            ...
            try {
              ...
              imgEl.src = 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300"%3E%3Crect fill="%23fee" width="400" height="300"/%3E%3Ctext fill="%23c00" x="50%" y="50%" text-anchor="middle" dy=".3em"%3EError%3C/text%3E%3C/svg%3E'
            }
          }
        }
      },
