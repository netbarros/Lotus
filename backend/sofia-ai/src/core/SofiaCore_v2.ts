...
export class SofiaCore_v2 {
  ...
  async decideComponent(
    ...
    try {
      ...
      const traditionalAnalyses = await Promise.all(
        versions.map(v => this.analyzeVersion(v))
      )
      ...
      logger.info(`  ✅ Decision: ${best.demo} (score: ${best.combinedScore.toFixed(2)}, confidence: ${(confidence * 100).toFixed(1)}%)`)
  ...
  private async analyzeVersion(version: any): Promise<AnalysisResult> {
    ...
    metrics.analysisLatency.observe(
      { type: 'version' },
  ...
  private async handleHealthCheck(data: any): Promise<void> {
    ...
    await this.redis.publish('mesh:health-response', JSON.stringify({
      agent: 'sofia-core-v2',
      health,
      timestamp: new Date()
    }))
  ...
  private async triggerSelfHealing(): Promise<void> {
    ...
    await this.redis.publish('mesh:request-optimization', JSON.stringify({
  ...
  private getCacheKey(component: string, versions: any[]): string {
    const demosList = versions.map(v => v.demo).sort().join(',')
