...
class QdrantService {
    ...
    async initialize() {
        ...
        try {
            ...
            logger_js_1.logger.info(`🔍 Connecting to Qdrant at ${this.config.host}:${this.config.port}...`);
    ...
    async createCollection(name, config) {
        ...
        try {
            ...
            logger_js_1.logger.info(`✅ Created collection: ${name} (${config.vectors.size}D, ${config.vectors.distance})`);
            // Record event
            await this.eventStore.record({
    ...
    async upsert(collectionName, points) {
        ...
        logger_js_1.logger.info(`🔍 Upserting ${points.length} points to collection: ${collectionName}`);
        try {
            // Simplified implementation - in production, use actual Qdrant client
